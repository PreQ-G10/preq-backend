...
private val locationRepository: LocationRepository,
    ...
    fun searchNearby(latitude: Double, longitude: Double): List<LocationSearchResponse> {
        return locationRepository.findNearby(latitude, longitude)
    ...
    fun register(request: BusinessRegisterRequest): BusinessRegisterResponse {
        ...
        val location = if (request.locationId != null) {
            findAndValidateLocation(request.locationId)
        } else {
            createLocation(request)
        }

        val registerUserResponse = authService.register(
            RegisterRequest(
        ...
        locationRepository.save(location.apply {
            claimedBy = user
    ...
    fun getProfile(user: User): BusinessProfileResponse {
        val location = locationRepository.findByClaimedBy(user)
    ...
    fun updateProfile(user: User, request: UpdateBusinessProfileRequest): BusinessProfileResponse {
        val location = locationRepository.findByClaimedBy(user)
            ?: throw EntityNotFoundException("No location found for this business account")
