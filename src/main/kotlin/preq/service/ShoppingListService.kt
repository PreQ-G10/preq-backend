...
class ShoppingListService(
    ...
    fun save(
        request: SaveShoppingListRequest,
        user: User,
        ...
        val list =
            ShoppingList().apply {
                ...
                this.totalPrice =
                    request.items.fold(BigDecimal.ZERO) { acc, item ->
    ...
    fun update(
        ...
        val list =
            shoppingListRepository.findByIdAndUserId(id, user.id)
        ...
        request.items.forEach { itemReq ->
            val item =
                shoppingListItemRepository.findByIdAndShoppingListId(itemReq.itemId, id)
    ...
    fun getAll(user: User): List<ShoppingListSummaryResponse> = shoppingListRepository.findSummariesByUserId(user.id)
    ...
    fun getBusinessMetrics(user: User): BusinessMetricsResponse {
        val location =
            locationRepository.findByClaimedById(user.id)
