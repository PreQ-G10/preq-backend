...
class ProductService(
    ...
    fun confirmMatch(productId: Long, file: MultipartFile, similarity: Double): Product {
        val product = productRepository.findById(productId).orElseThrow()
        val embedding = imageEmbeddingService.generateEmbedding(file)
        val imageUrl = cloudinaryService.upload(file)

        val status = if (similarity >= confidenceThreshold)
            ProductImageStatus.APPROVED else ProductImageStatus.PENDING_REVIEW

        product.images.add(ProductImage().apply {
